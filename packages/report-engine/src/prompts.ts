...
/** (C) 급여·4대보험 리포트용 도메인 헤더(고정). */
export const PAYROLL_DOMAIN_HEADER = `[작업: 급여 / 4대보험 / 실수령액 리포트]
입력 CRO는 직원별 4대보험 공제·실수령액과 집계를 담는다(domain: payroll).
- flags[]의 식대 비과세 한도 초과·공제율 비정상을 finding으로 우선 다룬다.
- 직원별 수치(공제·실수령)는 emp.* metric을 그대로 인용한다. 임의로 합산·추정하지 마라.
- 4대보험은 코드가 요율로 산출한 값이다. 요율의 적정성·법령 부합 여부를 단정하지 말고 "확인 포인트"로 제시한다.
- 전월 대비 급변동·공제율 이상은 "왜 그런지" 가설(인원 변동·수당 신설·정산 반영 등)을 검증 대상으로 쓴다.
- 지급 확정·정정 여부 판단은 하지 않는다. 사람이 검토·승인하도록 이상 항목과 확인 포인트만 제시한다.`;

export type ReportKind = 'cash' | 'closing' | 'payroll';
